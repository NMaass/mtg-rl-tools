"""Deterministic agent and verification interface over the pinned XMage engine."""
...
from magic_cabt.agents import is_legal_selection, make_agent
...
def _canonical_string(value, identities):
    def replace(match):
        raw = match.group(0)
        return identities.get(raw, "[runtime-id]")
    return _UUID.sub(replace, value)


def _canonical_value(value, identities):
    if isinstance(value, dict):
        result = {}
        for key, nested in value.items():
            if key in _RUNTIME_ID_FIELDS:
                if nested is None:
                    ...
                    result[key] = None
                continue
            result[key] = _canonical_value(nested, identities)
        return result
    if isinstance(value, list):
        return [_canonical_value(item, identities) for item in value]
    if isinstance(value, str):
        return _canonical_string(value, identities)
    return value


def canonical_observation(observation):
    """Return the deterministic agent view without process-local UUIDs."""
...
def canonical_result(result):
    ...
    final = canonical_observation(synthetic)["current"]
    winner = result.get("winner")
    return {
        "winner": _canonical_string(str(winner), {}) if winner is not None else None,
        "finalState": final,
    }


class NativeSession:
    """One deterministic game process with separate agent and verifier state."""

    def __init__(self, spec, bridge_factory=CabtBridge):
        if (not isinstance(spec, dict) or not isinstance(spec.get("decks"), list)
                or len(spec["decks"]) != 2):
        ...
        self.spec = {
            "decks": copy.deepcopy(spec["decks"]),
            "seed": seed,
            "maxTurns": turns,
        }
        ...
        self.failed = False
        try:
            self.response = self.bridge.game_start(
                *self.spec["decks"], seed=seed, max_turns=turns
            )
        except Exception:
            self.close()
            raise
        ...
        for index, option in enumerate(options):
            ...
            if action_id in index_by_id:
                raise ReplayDivergenceError(
                    "Current prompt contains duplicate stable action ids."
                )
            index_by_id[action_id] = index
        if (not isinstance(action_ids, list)
                or len(set(action_ids)) != len(action_ids)
                or any(not isinstance(action_id, str) or action_id not in index_by_id
                       for action_id in action_ids)):
            raise ReplayDivergenceError(
                "Recorded action id is duplicate or absent from the current prompt."
            )
    ...
    def restore(cls, checkpoint, bridge_factory=CabtBridge):
        if (not isinstance(checkpoint, dict)
                or checkpoint.get("kind") != "native-xmage-replay-root"
                or checkpoint.get("version") != 3
                or checkpoint.get("revision") != REFERENCE
                or checkpoint.get("setup") != SETUP):
            raise ValueError(
                "Only a native root from this engine and setup protocol can be restored."
            )
        steps = checkpoint.get("steps")
        expected = checkpoint.get("root")
        if (not isinstance(steps, list) or len(steps) > MAX_STEPS
                or not isinstance(expected, dict) or expected.get("finished") is not False):
        ...
        try:
            ...
            if (actual.get("finished")
                    or actual.get("fingerprint") != expected.get("fingerprint")
                    or actual_engine != expected.get("engineFingerprint")):
                raise ReplayDivergenceError(
                    "Rebuilt root does not match public semantics and private state.",
                    expected={
                        "public": expected.get("fingerprint"),
                        "engine": expected.get("engineFingerprint"),
                    },
                    actual={
                        "public": actual.get("fingerprint"),
    ...
    def autoplay(self, agent_specs=("random", "first"), decisions=100):
        if (len(agent_specs) != 2
                or any(agent not in ("random", "first") for agent in agent_specs)):
            raise ValueError(
                "Bundled controls are random and first. External agents use step."
            )
        if type(decisions) is not int or not 1 <= decisions <= MAX_STEPS:
            raise ValueError("Invalid decision budget.")
        agents = [
            make_agent(name, seed=self.spec["seed"] + index)
            for index, name in enumerate(agent_specs)
        ]
        frames = []
        ...
        for line in sys.stdin:
            ...
            try:
                ...
                if command in ("new", "restore"):
                    ...
                    session = None
                    session = (
                        NativeSession(request["spec"])
                        if command == "new"
                        else NativeSession.restore(request["checkpoint"])
                    )
                    ...
                    result = session.autoplay(
                        request.get("agents", ["random", "first"]),
                        request.get("decisions", 100),
                    )
                else:
                    raise ValueError("Unknown command.")
                print(json.dumps({"ok": True, "data": result}), flush=True)
            except (ValueError, KeyError, TypeError, ReplayDivergenceError,
                    CabtProtocolError, OSError) as error:
