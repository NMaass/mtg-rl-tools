...
body{font:14px system-ui;background:#111318;color:#eef2f7;margin:0}header{position:sticky;top:0;background:#111318;padding:16px;border-bottom:1px solid #333}main{padding:16px;overflow-x:auto}input,select,button{background:#1b1f27;color:#eef2f7;border:1px solid #444;padding:8px;margin-right:8px}button[aria-pressed='true']{background:#2b6cb0;border-color:#2b6cb0}table{border-collapse:collapse;width:100%%;min-width:1000px}th,td{border:1px solid #333;padding:8px;vertical-align:top}th{background:#181c23;position:sticky;top:82px}tr[hidden]{display:none}.disagree{background:#301b1b}.all-agree{background:#183022}.small{color:#9da8b7;font-size:12px}pre{white-space:pre-wrap}#count{margin-left:8px}#empty{color:#9da8b7;padding:24px;text-align:center}</style></head><body>
<header><h2>%(title)s</h2><input id='q' placeholder='Search'><select id='a'><option value=''>All</option><option value='disagree'>Disagreement</option><option value='all-agree'>All agree</option><option value='partial-agree'>Partial</option></select><button id='m' aria-pressed='false'>Human differs</button><span id='count' class='small'></span></header>
<main><table><thead><tr><th>Context</th><th>Human</th>%(headers)s<th>State/options</th></tr></thead><tbody id='rows'></tbody></table><p id='empty' hidden>No decisions match the current filters.</p></main>
<script id='data' type='application/json'>%(payload)s</script><script>
const data=JSON.parse(document.getElementById('data').textContent),body=document.getElementById('rows'),q=document.getElementById('q'),a=document.getElementById('a'),m=document.getElementById('m'),count=document.getElementById('count'),empty=document.getElementById('empty');let mistakes=false;const e=x=>String(x??'').replace(/[&<>"']/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[c]));function cell(x){if(!x.covered)return '<td>not scored</td>';return `<td><b>${e(x.topLabel)}</b><div class=small>${((x.topProbability||0)*100).toFixed(1)}%% · human rank ${e(x.playedRank)}</div><details><summary>top choices</summary><pre>${e(x.top.map((r,i)=>`${i+1}. ${r.label}`).join('\\n'))}</pre></details></td>`}
// Rows are built exactly once; filtering only flips row visibility. The old
// approach rebuilt the whole table per keystroke, which stuttered on
// thousand-decision bundles.
const frag=document.createDocumentFragment();const entries=data.rows.map(r=>{const tr=document.createElement('tr');tr.className=r.agreement;tr.innerHTML=`<td>turn ${e(r.turn)} ${e(r.phase)}<div class=small>${e(r.promptType)} · ${e(r.agreement)}</div></td><td><b>${e(r.human.label)}</b></td>${r.models.map(cell).join('')}<td>${e(r.stateSummary)}<details><summary>legal options</summary><pre>${e(r.options.map((o,i)=>`${i}. ${o.label}`).join('\\n'))}</pre></details></td>`;frag.appendChild(tr);return{r,tr,text:JSON.stringify(r).toLowerCase()}});body.appendChild(frag);
function apply(){const needle=q.value.toLowerCase();let shown=0;for(const {r,tr,text} of entries){const hide=(a.value&&r.agreement!==a.value)||(needle&&!text.includes(needle))||(mistakes&&!r.models.some(x=>x.covered&&x.playedRank!==1));tr.hidden=hide;if(!hide)shown++}count.textContent=`${shown} of ${entries.length} decisions`;empty.hidden=shown!==0}
let t;q.oninput=()=>{clearTimeout(t);t=setTimeout(apply,120)};a.onchange=apply;m.onclick=()=>{mistakes=!mistakes;m.setAttribute('aria-pressed',String(mistakes));apply()};apply();
